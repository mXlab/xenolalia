...
class TestPostprocessOutput(unittest.TestCase):
    ...
    def test_solid_white_becomes_border_only(self):
        ...
        result = xeno_image.postprocess_output(img, output_size=64, line_width=1)
        arr = np.array(result)
        # The very centre must be black
        cx, cy = 32, 32
        self.assertEqual(arr[cy, cx], 0,
    ...
    def test_line_width_zero_produces_thinner_result_than_line_width_two(self):
        ...
        r2 = xeno_image.postprocess_output(img, output_size=112, line_width=4)
        lit0 = np.count_nonzero(np.array(r0))
        lit2 = np.count_nonzero(np.array(r2))
        self.assertLess(lit0, lit2,
